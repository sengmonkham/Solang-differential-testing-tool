...
fn compile_solidity(src: &str) -> (Vec<u8>, String) {
    let mut sol_file = tempfile::NamedTempFile::with_suffix(".sol")
        .expect("could not create temp file");
    sol_file.write_all(src.as_bytes()).expect("could not write source");
    ...
    let solang_bin = std::env::var("SOLANG_BIN")
        .unwrap_or_else(|_| "solang".to_string());
    ...
    if !output.status.success() {
        eprintln!("ERROR: solang compile failed:\n{}",
            String::from_utf8_lossy(&output.stderr));
        std::process::exit(1);
    }
    if !output.stderr.is_empty() {
        eprintln!("[solang warnings]\n{}", String::from_utf8_lossy(&output.stderr));
    }

    let wasm_entry = std::fs::read_dir(&out_path).unwrap()
        .filter_map(|e| e.ok())
...
fn measure(env: &Env, addr: &Address, func_name: &str, extra_args: &[Val]) -> Metrics {
    ...
    for arg in extra_args { soroban_args.push_back(arg.clone()); }
...
fn main() {
    let rust_wasm_path = std::env::args().nth(1)
    ...
    println!("      ✓ WASM size: {} bytes", format_num(sol_wasm.len() as u64));
    #[allow(deprecated)]
    ...
    println!("      ✓ WASM size: {} bytes", format_num(rust_wasm.len() as u64));
    #[allow(deprecated)]
...
fn print_report(func: &str, sol: &Metrics, rust: &Metrics) {
    ...
    let match_str = if sol.return_val.shallow_eq(&rust.return_val) { "✓ yes" } else { "✗ MISMATCH" };

    println!("┌─────────────────────────┬────────────────────┬────────────────────┐");
    println!("│  {:<23} │  Solang (Solidity)  │  Rust SDK          │", func);
    println!("├─────────────────────────┼────────────────────┼────────────────────┤");
    println!("│  CPU Instructions       │  {:<18} │  {:<18} │",
        format_num(sol.cpu_insns), format_num(rust.cpu_insns));
    println!("│  Memory (bytes)         │  {:<18} │  {:<18} │",
        format_num(sol.mem_bytes), format_num(rust.mem_bytes));
    println!("│  Return value match     │  {:<38} │", match_str);
    println!("├─────────────────────────┴────────────────────┴────────────────────┤");
    println!("│  Overhead:  {:.2}× CPU   {:.2}× memory                           │",
        cpu_ratio, mem_ratio);
    if cpu_ratio > 1.5 {
